library(ggplot2)
fdi <- read.csv("/home/krishna/Documents/DM/FDI/CSV/FDI.csv",header = TRUE)
industries <- fdi[,1] #First Column contains name of Industry
fdi <- fdi[,-1]
time_series <- seq(2000,2016,by = 1)

i = 1

all.plots <- vector(nrow(fdi),mode = 'list') #Storing all the Plots in a single List

#Plotting Histogram for each Industries in the Gross Domestic Product DataSet
for(i in 1:nrow(fdi)){
  fdi_per_industry <- fdi[i,]
  plot_data <- data.frame(years = time_series,fdi = as.numeric(fdi_per_industry))
  graph <- ggplot(plot_data,aes(years,fdi)) + geom_bar(stat = "identity",colour = "black",fill = "red") +
    ggtitle(industries[i]) + xlab("Time Series") + ylab("FDI (in crores)")
  plot(graph)
  all.plots[[i]] <- recordPlot() 
}

# pdf("/home/krishna/Documents/DM/FDI/Output/Histogram_FDI.pdf",onefile = TRUE)
for(plot in all.plots){
  replayPlot(plot)
}
graphics.off()